...
def make_problem(prob, instance, ed, et):
    ...
    prob.set_objective("max", prob.sum([s[i] * instance.interest[i] for i in range(n)]))

    # Distance bound
    prob.add_constraint(prob.sum([x[i, j] * instance.dist[i-1][j-1] for j in range(1, n+1) for i in range(1, n+1)]) <= ed, ctname="distance_epsilon")

    # Time bound
    prob.add_constraint(prob.sum([s[i] * instance.time[i] for i in range(n)]) <= et, ctname="time_epsilon")

    # inbound = outbound = s
    for j in range(1, n + 1):
        prob.add_constraint(prob.sum([x[i, j] for i in range(n + 1)]) == s[j - 1], ctname='inbound' + str(j))

    for i in range(1, n + 1):
        prob.add_constraint(prob.sum([x[i, j] for j in range(1, n + 2)]) == s[i - 1], ctname='outbound' + str(i))

    # only one arc leaves start only on enters end
    prob.add_constraint(prob.sum([x[0, i] for i in range(1, n + 1)]) == 1, ctname="path_start")
    prob.add_constraint(prob.sum([x[i, n + 1] for i in range(1, n + 1)]) == 1, ctname="path_end")

    # no self loops , no start->end
    prob.add_constraint(x[0, n + 1] == 0, ctname="path_structure_0")
    prob.add_constraint(prob.sum([x[i, i] for i in range(0, n+2)]) == 0, ctname="path_structure_1")
...
if __name__ == '__main__':
    ist = TapInstance("/home/alex/instances/tap_12_400.dat")

    budget = round(0.25 * ist.size * 27.5)
    dist_bound = round(0.35 * ist.size * 4.5)

    ist.interest = [i * 1000 for i in ist.interest]
